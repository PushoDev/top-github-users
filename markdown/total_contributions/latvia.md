...
The `public contributions` and `private contributions` by users in Latvia on `2023/12/3 4:09 AM UTC`. This list contains users from `Latvia` and cities `Riga` `Daugavpils` `Liepāja` `Jūrmala` `Ventspils` `Jelgava` `Cēsis`.

There are `138 countries` and `674 cities` can be found [here](https://github.com/PushoDev/top-github-users).

There are `957 users`  in Latvia. You need at least `4 followers` to be on this list.
